...
namespace Utility {
    ...
    /*
     * Various assorted utilities functions Made by codemonkey
     * */
    public static class Utils {
        ...
        public static TextMesh CreateWorldTextXZ(string text, Transform parent = null, Vector3 localPosition = default(Vector3), int fontSize = 40, Color? color = null, TextAnchor textAnchor = TextAnchor.UpperLeft, TextAlignment textAlignment = TextAlignment.Left, int sortingOrder = sortingOrderDefault) 
        {
            if (color == null) color = Color.white;
            return CreateWorldTextXZ(parent, text, localPosition, fontSize, (Color)color, textAnchor, textAlignment, sortingOrder);
        }

        public static TextMesh CreateWorldTextXZ(Transform parent, string text, Vector3 localPosition, int fontSize, Color color, TextAnchor textAnchor, TextAlignment textAlignment, int sortingOrder) 
        {
            GameObject gameObject = new GameObject("World_Text", typeof(TextMesh));
            Transform transform = gameObject.transform;
        ...
        public static FunctionUpdater CreateMouseDraggingAction(int mouseButton, Action<Vector3> onMouseDragging) {
            ...
            return FunctionUpdater.Create(() => {
                ...
                if (dragging) {
                    onMouseDragging(Utils.GetMouseWorldPosition());
                }
                return false; 
            });
        }
        ...
        public static FunctionUpdater CreateMouseClickFromToAction(int mouseButton, int cancelMouseButton, Action<Vector3, Vector3> onMouseClickFromTo, Action<Vector3, Vector3> onWaitingForToPosition) {
            ...
            return FunctionUpdater.Create(() => {
                if (state == 1) {
                    if (onWaitingForToPosition != null) onWaitingForToPosition(from, Utils.GetMouseWorldPosition());
                }
                if (state == 1 && Input.GetMouseButtonDown(cancelMouseButton)) {
                    // Cancel
                    state = 0;
                }
                if (Input.GetMouseButtonDown(mouseButton) && !Utils.IsPointerOverUI()) {
                    if (state == 0) {
                        state = 1;
                        from = Utils.GetMouseWorldPosition();
                    } else {
                        state = 0;
                        onMouseClickFromTo(from, Utils.GetMouseWorldPosition());
                    }
                }
                return false; 
            });
